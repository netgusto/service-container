...
Container.prototype.constructService = function constructService(id, isOptional, serviceTree, ns) {
  ...
  if (definition.isFunction) {
    svc = definition.class.apply(svc, arguments);
 ...
 * @returns {Boolean} Returns true if the parameter exists
 */
Container.prototype.getServiceIdsHavingTag = function (tagname) {

  var services = {};
  for(var serviceId in this.serviceDefinitions) {

    if(this.serviceDefinitions[serviceId].hasTag(tagname)) {
      services[serviceId] = {
        id: serviceId,
        attributes: this.serviceDefinitions[serviceId].getTag(tagname)
      };
    }
  }
